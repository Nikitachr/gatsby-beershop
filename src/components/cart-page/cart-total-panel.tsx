    ...
    return (
        <div className={`${className} rounded-3xl px-4 py-5 border-2 border-secondary h-fit`}>
            ...
            <div className="grid my-7 gap-4">
                {products.map(({ product, amount }) => <div key={product.id} className="flex justify-between gap-2">
                    <p>{product.title}</p>
            ...
            <button aria-label="Checkout" className="rounded-full bg-blue hover:bg-blue-light text-white text-xl mt-7 w-full p-3">Checkout</button>
        </div>
    );
};
